...
library(corrplot)
white_wine_data_clean <- na.omit(white_wine_data[, c("citric.acid", "chlorides","density",  "alcohol")])

red_wine_data_clean <- na.omit(red_wine_data[, c("citric.acid", "chlorides","density", "alcohol")])

correlation_matrix_white <- cor(white_wine_data_clean)
correlation_matrix_red <- cor(red_wine_data_clean)

combined_correlation_matrix <- (correlation_matrix_white + correlation_matrix_red) / 2

corrplot(correlation_matrix_white, method = "color", type = "upper", tl.col = "black", tl.srt = 45, addCoef.col = "red")
...
Data cleaning is performed on both white_wine_data and red_wine_data in the above code, deleting rows with missing values in certain columns.
...
A correlation plot with a customized appearance is built for the white wine dataset.
