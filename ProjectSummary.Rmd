...
3. The boxplot shows that wines with higher quality seem to have a higher alcohol content.
...
3. The boxplot shows that wines with higher quality seem to have a less denser.
...
3. The boxplot shows that wines with higher quality seem to have a less chlorides. 
...
We found that the p-value is less than 0.05, we reject the null hypothesis and conclude that there is a significant difference in mean alcohol concentration between red and white wine.
...
NULL HYPOTHESIS (H0): There is no significant mean difference in density between red and white wine.
ALTERNATE HYPOTHESIS (H1): There is a significant mean difference between the two wines.

Perform the Welch two-sample t-test for density and calculate the p-value. If the p-value is less than 0.05, reject the null hypothesis and conclude that there is a significant difference in mean density between red and white wine.

ANOVA TEST FOR RED AND WHITE WINE:

NULL HYPOTHESIS (H0): There is no significant difference in mean density across quality categories in red wine/white wine.
ALTERNATE HYPOTHESIS (H1): There is a significant difference in mean density across quality categories in red wine/white wine.

Perform separate ANOVA tests for red and white wine to determine if there is a significant difference in mean density across all quality categories for each wine. If the p-value for either red or white wine is less than 0.05, reject the null hypothesis and conclude that there is a significant difference in mean density across all quality categories.
...
Although we have a very descriptive data set that is perfect for our analysis, we have to mention that the amount of data present for white wine is comparatively more that red wine. Also, majority of the data has quality ratings between 4-6. Less observations were found for quality ratings of 8 and 9. 

Having a more balanced data set will definitely improve our analysis.

----------------------------------------------------------------------------------------------

## Conclusions

From our analysis we definitely did find something interesting! Not just the alcohol content or the sugar or acidity but factors such as density and chlorides make a substantial difference in the quality of wine and also are one among the factors that makes red and white wine standout from each other!

Now You know which and what kind of wine to buy!!

----------------------------------------------------------------------------------------------

## Additional Insights

As interesting as it was performing these analysis, we would like to enhance further more to find better results. We would want to expand our data set with more attributes and work on a prediction model that will provide more comprehensive results. 
...
1. P. Cortez, A. Cerdeira, F. Almeida, T. Matos and J. Reis. Modeling wine preferences by data mining from physiochemical properties. In Decision Support Systems, Elsevier, 47(4):547-553, 2009.↩

The dataset is also avalible on kaggle.
